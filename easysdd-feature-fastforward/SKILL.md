...
本技能在正文里用自然语言术语引用路径（**design.md**、**feature 目录**），具体目录位置见主技能 `easysdd` 第二节"目录安排"——那是整个 easysdd 家族目录约定的唯一定义处。

> `{feature}` 是占位符，代表具体 feature 的**目录名**，格式是 `YYYY-MM-DD-{英文 slug}`（日期前缀用于排序，小写连字符 slug）。
