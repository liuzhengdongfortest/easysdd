...
> 共享路径与命名约定以根技能 `easysdd-core` 第二节为准。本节只补充本阶段特有信息：到本阶段，feature 目录通常已由 brainstorm 阶段创建好；若不存在则在本阶段创建。
...
4. **归档检索（按需）**——是否值得搜、优先搜哪些目录，以根技能 `easysdd-core` 第五节约束 7 为准；本阶段至少优先考虑以下目录：
...
方案 doc 确认后，从 design.md 中提取行动清单，落盘为同目录下的 `checklist.yaml`。这份清单的生命周期以 `easysdd-core/reference/shared-conventions.md` 为准：本阶段负责生成，implement 只推进 `steps`，acceptance 只核对 `checks`。
