...
> 共享路径与命名约定以根技能 `easysdd-core` 第二节为准。本节只补充本阶段特有信息：到本阶段，feature 目录已由 brainstorm 或 design 阶段创建好。
...
2. **checklist.yaml 存在且有效**。`checklist.yaml` 的生命周期以 `easysdd-core/reference/shared-conventions.md` 为准；本阶段只消费并推进 `steps`。检查同 feature 目录下的 `checklist.yaml`:
...
- 按 `checklist.yaml` 的 `steps` 列表顺序执行,不合并步骤,不跳步
- 每完成一步,立即把该步 `status` 从 `pending` 改为 `done`
