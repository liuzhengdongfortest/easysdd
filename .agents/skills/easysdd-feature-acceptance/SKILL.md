...
> 共享路径与命名约定以根技能 `easysdd-core` 第二节为准。本节只补充本阶段特有信息。
...
3. **checklist.yaml 存在且 steps 已完成**。`checklist.yaml` 的生命周期以 `easysdd-core/reference/shared-conventions.md` 为准；本阶段只核对并更新 `checks`。检查同 feature 目录下的 `checklist.yaml`:
