...
技巧文档的 frontmatter、正文模板和长示例已拆到 `easysdd-tricks/reference.md`。本技能只保留判断与流程规则：

- `type` 仍然只允许 `pattern` / `library` / `technique`
- 示例优先使用项目里的真实代码或命令
- `何时不适用`、`已知坑`、`相关文档` 仍然是可选节，用户说"没什么"就省略
...
1. 这条技巧是否应该在 `AGENTS.md` 或 `AGENTS.md` 里加一行"遇到 X 情况，可检索技巧库"——让 AI 未来能主动检索到它
...
> 完整语法和示例见 `easysdd-core/reference/tools.md`。本节只列 tricks 特有的典型查询。
...
完整示例已拆到 `easysdd-tricks/reference.md`。本技能正文不再内嵌长示例，避免技巧流程被示例淹没。
