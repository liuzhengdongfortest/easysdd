...
> 共享路径与命名约定以根技能 `easysdd-core` 第二节为准。本节只补充 fastforward 特有信息。
...
**确认后，立即从 design.md 提取行动清单，落盘为同目录下的 `checklist.yaml`**。清单格式和生命周期以 `easysdd-core/reference/shared-conventions.md` 为准；本阶段只负责初始生成。提取规则：
