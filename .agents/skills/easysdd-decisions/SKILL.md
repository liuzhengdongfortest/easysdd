...
> 完整语法和示例见 `easysdd-core/reference/tools.md`。本节只列 decisions 特有的典型查询。
...
| `easysdd-feature-acceptance` 结束 → 可选推荐 | 验收完成后，按 `easysdd-core/reference/shared-conventions.md` 的收尾推荐判断：如果这次 feature 引入了新约定或技术，推荐记录一次 |
...
> 归档类工作流共享守护规则（只增不删、宁缺毋滥、不替用户写、可发现性、归档后查重叠）见根技能 `easysdd-core` 第五节约束 10。以下为本技能特有或细化规则：
