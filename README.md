...
<tr><td><b>路线图</b></td><td><code>cs-roadmap</code></td><td>把模糊大目标拆成可推进的子任务序列</td></tr>
<tr><td rowspan="5"><b>特性流程</b></td><td><code>cs-feat</code></td><td>新特性子流程入口</td></tr>
<tr><td><code>cs-brainstorm</code></td><td>想法还模糊时，对话理清到能进 design</td></tr>
...
```
                              ┌──────────────┐
                              │  cs-onboard  │  接入 / 同步参考文档
                              └──────┬───────┘
                                     ↓
        ┌────────────────┬───────────┴────────────┬────────────────┐
        ↓                ↓                        ↓                ↓
   ┌─────────┐     ┌──────────┐            ┌──────────┐      ┌──────────┐
   │ cs-req  │ ──> │ cs-arch  │ ──> roadmap│ feature  │ bug ─│  issue   │
   └─────────┘     └──────────┘            │  流程    │      │  流程    │
                                           └────┬─────┘      └────┬─────┘
                                                ↓                 ↓
                                     design → impl → accept   report → analyze → fix
                                                ↓                 ↓
                                                └──────┬──────────┘
                                                       ↓
                                            ┌────────────────────┐
