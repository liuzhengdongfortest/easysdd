...
5. **确认落定**——用户确认后批量改 `status: current`
...
1. `search-yaml.py` 搜 `status=outdated` 的条目，或对比 `_manifest.yaml`；也可以 `--sort-by last_reviewed --order asc` 直接按最久没复核的排在前，挑"应该看一下是不是还准"的条目
...
| `easysdd-architecture-check` | 接口变更但 libdoc 未同步 → 标对应条目 `outdated` |
