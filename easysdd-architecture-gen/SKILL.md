...
4. **术语防撞车做了吗？**——新引入的架构术语做一遍 grep（代码、`easysdd/architecture/` 下所有文档、`easysdd/compound/`）。撞了就换名字或在第 0 节明确"本文里 X 指 Y，和代码里的 X' 不是一个东西"。
...
- 术语撞车：新引入的名字和代码 / 其他 architecture doc / compound 里已有的东西撞了还不自知
